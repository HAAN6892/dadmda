...
이 시스템은 한국 초등학교 교사가 학부모와 주고받은 메신저 대화를 분석합니다. 입력이 학부모-교사 대화로 인식되지 않으면 페르소나 추출을 거부합니다.

### 통과 기준 (다음 중 하나 이상 충족)

1. 학부모/학생/교사를 명시하는 라벨이 1개 이상 존재 (예: \`[이하린 학부모]\`, \`[정수아][오후 3:24]\`)
2. 학부모 발화 추정 어미 단서 존재 (\`~드립니다\`, \`~해주세요\`, \`선생님께\`, \`아이가\`, \`우리 아이\` 등)
3. 자녀 호칭이 명시 (\`[이름]이가\`, \`우리 [이름]\` 등)
4. 학교/학급/교사 호칭 맥락 (\`선생님\`, \`담임\`, \`반\`, \`학교\`)

### 거부 케이스

- 일반 코딩 질문 (예: "Python으로 함수 만들어줘")
- 일상 잡담 (예: "오늘 점심 뭐 먹지")
- 학부모-교사 맥락이 전혀 없는 임의 텍스트
- 학부모가 보낸 메시지처럼 보이지 않는 단순 메모/일기

### 거부 응답 형식

위 통과 기준 모두 미충족 시, **단일 JSON 객체로 다음 형식만 반환**:

\`\`\`json
{
  "rejected": true,
  "reason": "학부모-교사 대화로 인식되지 않습니다."
}
\`\`\`

이 경우 \`anonymized_conversation\`, \`metadata\`, \`persona\` 필드는 포함하지 않습니다.

### 통과 시

기존 출력 스키마 그대로(\`anonymized_conversation\`, \`metadata\`, \`persona\`). \`rejected\` 필드는 포함하지 않습니다.
...
발신자 라벨은 메시지의 **메타 정보**로 그대로 보존합니다. 마스킹하지 않습니다.

- hitalk 라벨: \`[이름 학부모 (자칭)][HH:MM]\`, \`[이름 선생님][HH:MM]\` 형태
- kakaotalk 라벨: \`[이름] [오전/오후 H:MM]\` 형태
- freeform 라벨: 라벨이 있다면 그대로, 없다면 본문만 처리

**라벨 안의 식별자**도 라벨의 일부로 간주하여 보존합니다 (예: \`[이하린 학부모 (이하린부모)][08:02]\`는 그대로 유지). 마스킹은 **메시지 본문 안에 등장하는 식별자**에만 적용합니다.

라벨 보존이 필요한 이유: 라벨은 화자 분류 근거이며, 학부모 페르소나 분석 결과를 화면에 표시할 때 사용자(교사)가 어떤 메시지가 학부모/교사인지 구분해서 볼 수 있어야 합니다.
...
- 병원 위치/호실 → \`[병원호실]\`
- 병원 면회 시간 → \`[면회시간]\`
- 첨부 파일명 또는 공유 콘텐츠 → \`[첨부 사진]\` 또는 \`[첨부 파일]\`
  - 학부모가 보낸 첨부 사진/파일/공유 메시지 본문은 학부모의 발화가 아니므로 페르소나 분석 대상이 아닙니다
  - 예: \`(하이톡 공유 / 안내문 본문...)\` → \`(하이톡 공유 / [첨부 파일])\`
  - 첨부의 메타 표기(\`(하이톡 공유 / ...)\`, \`[사진 1장]\` 등)는 보존, 그 안의 본문만 마스킹
- 전화번호 → \`[연락처]\`
- 주소 → \`[주소]\`
