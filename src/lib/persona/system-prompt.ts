...
\`speaker_classification_note\`에 어떤 메시지가 누구 메시지로 분류됐는지 간단히 적습니다. 학생 메시지만 있고 학부모 메시지가 없으면 그대로 처리하되 \`relationship_type\`은 결정 안 하고 null로 둡니다.

#### \`speaker_mixed\` 플래그 결정 규칙

\`speaker_mixed\`는 **페르소나 추출 대상(학부모 메시지)을 다른 발신자 메시지와 구분 가능한지**를 표기하는 플래그입니다. 교사 메시지가 섞여있는지 여부와는 무관합니다.

- **\`speaker_mixed = false\`** (페르소나 추출 대상 명확)
  - 학부모 메시지가 라벨/형식으로 명확히 식별 가능한 경우
  - 교사 메시지가 함께 있어도 라벨로 분리되어 학부모 메시지만 골라낼 수 있으면 \`false\`
  - 예: hitalk 형식에서 \`[이름 학부모 (자칭)]\`과 \`[이름 선생님]\` 라벨이 구분되는 경우

- **\`speaker_mixed = true\`** (페르소나 추출 대상 모호)
  - 학생 라벨로 임포트됐는데 본문이 학부모 메시지로 보이는 경우 (학생 라벨에 학부모 메시지 혼합)
  - 발신자 라벨이 없어 학부모/학생 구분이 불가능한 자유 형식
  - 한 라벨 안에 여러 화자 메시지가 섞여 있어 학부모 메시지를 분리할 수 없는 경우

**핵심 원칙**: 교사 메시지가 함께 있는 것은 정상 동작이며 \`speaker_mixed\`에 영향을 주지 않습니다. 교사 메시지는 페르소나 추출 대상이 아니지만 발신자 분류는 명확하므로 학부모 메시지만 별도로 추출하면 됩니다. \`speaker_mixed = true\`는 학부모/학생 메시지 구분 자체가 모호한 경우에만 사용합니다.

자유 형식의 경우 발신자 라벨이 불분명할 수 있습니다. 가능한 단서(어미, 호칭, 내용 맥락)로 추론하되 학부모 메시지를 식별할 단서가 부족하면 \`speaker_mixed = true\`로 표기합니다.
